...
gplot + theme(axis.text.x=element_text(hjust=1)) + coord_flip() + theme_igray() +  scale_fill_pander()+ xlab("Job Role") + ylab("Count of Employees")
...
gplot2 <- ggplot(employee, aes(x = EducationField, ..count.., fill = factor(Attrition))) + geom_bar(position="dodge")+ xlab("Education Field") + ylab("Count of Employees")
...
# Plots of each of the survey respones and their totals. Surprisingly no difference in atrition rates.
```{r}
ggplot(employee, aes(EnvironmentSatisfaction, fill = Attrition)) + geom_bar() + scale_fill_solarized() +  theme_solarized() + labs(title="Attrition Based on Environment Satisfaction Responses", x="Environment Satisfaction Rating", y="Count of Employees")

ggplot(employee, aes(JobSatisfaction, fill = Attrition)) + geom_bar(aes()) + scale_fill_solarized() +  theme_solarized() + labs(title="Attrition Based on Job Satisfaction Responses", x="Job Satisfaction Rating", y="Count of Employees")

ggplot(employee, aes(RelationshipSatisfaction, fill = Attrition)) + geom_bar(aes()) + scale_fill_solarized() +  theme_solarized() + labs(title="Attrition Based on Relationship Satisfaction Responses", x="Relationship Satisfaction Rating", y="Count of Employees")
```
#Hourly Rate and Business Travel
```{r}
HrlyRate<- cut(employee$HourlyRate, 7, include.lowest = TRUE)
 ggplot(employee, aes(HrlyRate, ..count.., fill = factor(Attrition))) + geom_bar(position="stack", width = 0.5) + scale_fill_manual(values=c("green","Royalblue"))
 ggplot(employee, aes(BusinessTravel, ..count.., fill = factor(Attrition))) + geom_bar(position="stack", width = 0.5) + scale_fill_manual(values=c("green","Royalblue"))
```
...
#Changing Attirtion to a 1 and 0 to be able to divide and converting the odd Age column into  readable field
AttritionData$Attrition2[AttritionData$Attrition == "Yes"] <- 1
AttritionData$Attrition2[AttritionData$Attrition == "No"] <- 0
AttritionData$Age <- AttritionData$�..Age
...
#Converting AGe into a time series to procude a dygraph
...
dyShading(from = "24", to = "26", color = "#DCDCDC") %>%
dyShading(from = "35", to = "51", color = "#DCDCDC")

#values of total employees by age
...
#job level information to output into chart
...
boxplot(AttritionCalcs$TotalAttritionScore)
summary(AttritionCalcs$TotalAttritionScore)
```
Leaving out of r so not to repeat over and over
